    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      hls::stream<result_t> layer2_out("layer2_out");
            ...
            if (e % CHECKPOINT == 0) {
                ...
                for(int i = 0; i < N_SIZE_0_2*N_SIZE_1_2*N_SIZE_2_2; i++) {
                  std::cout << pr[i] << " ";
                }
                std::cout << std::endl;
                std::cout << "Quantized predictions" << std::endl;
                // hls-fpga-machine-learning insert quantized
                nnet::print_result<result_t, N_SIZE_0_2*N_SIZE_1_2*N_SIZE_2_2>(layer2_out, std::cout, true);
            }
            e++;

            // hls-fpga-machine-learning insert tb-output
            nnet::print_result<result_t, N_SIZE_0_2*N_SIZE_1_2*N_SIZE_2_2>(layer2_out, fout);
        }
        fin.close();
        fpr.close();
    } else {
